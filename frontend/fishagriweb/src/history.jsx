...
const [filterOptions, setFilterOptions] = useState({ date: null, time: null });

useEffect(() => {
    const fetchHistoryRecords = async () => {
        try {
            const response = await fetch('http://localhost:3002/historyrecord');
...
const filterAndSortRecords = () => {
    ...
    if (sortOption) {
        if (sortOption === 'phLow') {
            ...
            records.sort((a, b) => a.tur - b.tur);
        } else if (sortOption === 'turbidityHigh') {
            records.sort((a, b) => b.tur - a.tur);
...
const handleClearHistory = async () => {
    try {
        await fetch('http://localhost:3002/historyrecord', { method: 'DELETE' });
...
const handleDeleteRecord = async (id) => {
    try {
        await fetch(`http://localhost:3002/historyrecord/${id}`, { method: 'DELETE' });
...
const data = {
    ...
    datasets: [{
        data: [selectedRecord?.ph || 0, selectedRecord?.temp || 0, selectedRecord?.tur || 0],
            ...
            <div className='back' style={{ overflowY: 'scroll', display: 'flex' }}>
                <div style={{ marginTop: '100px', marginLeft: '100px' }}>
                    <ul style={{ listStyleType: 'none'}}>
                        ...
                        <li>
                            <button 
                                style={{ display: 'flex', border: 0, backgroundColor: 'transparent', fontSize: '25px', color: '#1f639e', fontWeight: 'bold' }} 
                                onClick={handleClearHistory}
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="#46a2f5" style={{ width: '30px', height: '30px' }}>
                                    <path fill='#1f639e' d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"/>
                                </svg>
                                Clear History
                            </button>
                        </li>
                        ...
                        <li>
                            ...
                            <hr />
                            <button 
                                style={{ display: 'flex', border: 0, backgroundColor: 'transparent', fontSize: '25px', color: '#1f639e', borderRadius: '5px', fontWeight: 'bold' }} 
                            ...
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style={{ width: '30px', height: '30px' }}>
                                    <path fill="#1f639e" d="M3.9 54.9C10.5 40.9 24.5 32 40 32h432c15.5 0 29.5 8.9 36.1 22.9s4.6 30.5-5.2 42.2L336 284.3V432c0 10.6-5.5 20.4-14.5 26.1l-96 56c-9.9 5.8-22.1 5.8-32 0s-14.5-16.4-14.5-26.1V284.3L9.1 97.1C-.7 85.4-2.6 70.8 3.9 54.9z"/>
                                </svg>
                                Filters
                            </button>
                        </li>
                ...
                <div style={{ marginTop: '100px', marginLeft: '300px', position:'relative' }}>
                    {selectedRecord && (
                        <div style={{ width: '250px', height: '250px', marginBottom: '20px', marginLeft: '700px', position:"fixed" }}>
                    ...
                    <div style={{ width: '600px', height: '150px' }}>
                        {filteredRecords.length > 0 ? (
                            filteredRecords.map(record => (
                                <div key={record.id} style={{ margin: '10px', padding: '10px', border: '1px solid #46a2f5', borderRadius: '5px', cursor: 'pointer' }} onClick={() => handleRecordClick(record)}>
                                    <div style={{ display: 'flex' }}>
                                        <p style={{ fontWeight: 'bold', fontSize: '20px', marginRight: '5px' }}>{record.date}</p>
                                        <p style={{ fontWeight: 'bold', fontSize: '20px' }}>{record.time}</p>
                                        <button
                                        ...
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="#c24a42" style={{ width: '20px', height: '20px', marginLeft: '330px' }}>
