...
export default function Discover() {
    ...
    const fetchTanks = async () => {
      try {
        const response = await fetch(
          "http://localhost:9000/api/tank/alltanksrecords"
        );
  ...
  const addTank = (newTank) => {
    console.log("entered addTank function");
    setTanks((prevTanks) => [...prevTanks, { tank_no: newTank }]); // Update to use prevTanks
    console.log("new tank added", newTank);
    console.log("all tanks present", [...tanks, newTank]); // Log new tank with spread
    setAddingFormVisible(false);
  };
          ...
          {selectedTank ? (
            ...
            <>
              ...
              <p style={{ width: "40%", margin: "0 auto" }}>
                All tanks in the fish farm are displayed here with all of its
                details
              </p>
              <button
