...
export default function Signup() {
    ...
    const handleSubmit = async () => {
        ...
        if (!firstName || !lastName || !email || !password) {
            setMessage({ show: true, type: 'error', text: 'Please fill in all fields.' });
            return;
        }

        if (!validateEmail(email)) {
            setMessage({ show: true, type: 'error', text: 'Invalid email. Please use @gmail.com or @hotmail.com.' });
            return;
        }

        if (password.length < 8) {
            setMessage({ show: true, type: 'error', text: 'Password must be at least 8 characters long.' });
            return;
        }
        ...
        try {
            ...
            if (data.length > 0) {
                // User already exists
                setMessage({ show: true, type: 'info', text: 'This User Already exists in our database.' });
            } else {
                // Create new user
                const newUser = {
                    firstName,
                ...
                setMessage({ show: true, type: 'success', text: 'Please wait and an admin will accept your request in a few seconds.' });
            ...
            setMessage({ show: true, type: 'error', text: 'An error occurred. Please try again later.' });
        }
    };
    ...
    // Automatically close all messages after 5 seconds
    useEffect(() => {
        if (message.show) {
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backdropFilter: 'blur(50px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px', width: '650px', height: '800px' }}>
                        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'start' }}>
                            <h1 style={{ color: '#46a2f5' }}>Sign Up</h1>
                            <p style={{ color: '#46a2f5' }}>Already have an account?
                                <Link to="/login">
                            ...
                            <div style={{ display: 'flex' }}>
                                ...
                                <div style={{ marginRight: '20px', textAlign: 'start' }}>
                                    <p style={{ color: '#46a2f5', cursor: 'pointer' }} id="firstname" onClick={() => inputRefs.current['firstname'].focus()}>First Name</p>
                                    <input 
                                        type="text" 
                                        name="firstname" 
                                        id="firstname" 
                                ...
                                <div style={{ textAlign: 'start' }}>
                                    <p style={{ color: '#46a2f5', cursor: 'pointer' }} id="lastname" onClick={() => inputRefs.current['lastname'].focus()}>Last Name</p>
                                    <input 
                                        type="text" 
                                        name="lastname" 
                                        id="lastname" 
                            ...
                            <div style={{ textAlign: 'start' }}>
                                <p style={{ color: '#46a2f5', cursor: 'pointer' }} id="email" onClick={() => inputRefs.current['email'].focus()}>Your email</p>
                                <input
                                    type="text"
                                    name="email"
                                    id="email"
                            ...
                            <div>
                                <div style={{ display: 'flex' }}>
                                    <p style={{ color: '#46a2f5', cursor: 'pointer' }} id="pass" onClick={() => inputRefs.current['password'].focus()}>Your password</p>
                                    <button
