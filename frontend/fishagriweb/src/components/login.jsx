...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateEmail(email)) {
      setError({
        show: true,
        message: "Invalid email format. Please use @gmail.com or @hotmail.com.",
      });
      setTimeout(() => setError({ show: false }), 5000);
      return;
    }
    ...
    try {
      ...
      if (data.status === 200) {
        ...
        sessionStorage.setItem("isEmployee", data.data.isEmployee);
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          setMessage({
            show: true,
            type: "error",
            text: "User not found. Please head to the Sign Up page.",
          });
        } else {
          setMessage({
            show: true,
          ...
          >
            ...
            >
              ...
              {message.show && (
                <div
                  className={
                    message.type === "success" ? "successone" : "unvalidone"
                  }
                  style={{ display: "flex", textAlign: "start" }}
                >
                  <svg
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginTop: "20px", textAlign: "center" }}>
                  <button className="button logbtn" type="submit">
                    Log in
                  </button>
                </div>
              </form>
